exp1raw<-read.csv("workers/experiment1_workers.csv")
exp2raw<-read.csv("workers/experiment2_workers.csv")
exp1raw<-exp1raw[exp1raw$judgments_count>0,]
exp2raw<-exp2raw[exp2raw$judgments_count>0,]

exp1<-read.csv2("experiment1/comprehension_filtered_fromoo.csv")
exp2<-read.csv2("experiment2/experiment2_filter_headmovementcorrect.csv")
...
workers1raw<-exp1raw$worker_id
workers2raw<-exp2raw$worker_id

duplicatew3<-intersect(workers3,c(workers1raw,workers2raw))
duplicatew2<-intersect(workers2,workers1raw)
