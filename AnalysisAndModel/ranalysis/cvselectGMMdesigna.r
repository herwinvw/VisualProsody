...
result<-foreach (k=2:maxmixtures) %dopar%
{
  ...
  write.table(df, file=paste0("cvinfoa_",k,".csv"),col.names=FALSE,row.names=FALSE)
  
  writeGMMToXML(mixture, filename=paste0("gmmVoicedA",k,".xml"))
  saveRDS(mixture, file = paste0("gmmVoicedA",k,".rda"))  
  
  df
}
df<-do.call("rbind",result)
write.csv(df,file="cvinfoa.csv", row.names=FALSE)
stopCluster(cl)
