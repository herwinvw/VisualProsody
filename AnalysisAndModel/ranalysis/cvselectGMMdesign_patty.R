...
maxmixtures=15
result<-foreach (k=9:maxmixtures) %dopar%
{
  library(mixtools)
  ...
  write.table(mixture$loglik, file=paste0("patty_loglike_voiced_training_",k,".csv"),col.names=FALSE,row.names=FALSE)
  write.table(loglikes.cv, file=paste0("patty_loglike_voiced_cv_",k,".csv"),col.names=FALSE,row.names=FALSE)
}